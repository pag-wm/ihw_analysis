...
# notification - https://data.texas.gov/dataset/Texas-Commission-on-Environmental-Quality-NOR-Epis/2mm8-rqyi/about_data
...
DATASETS = {
    ...
    "notification": ("2mm8-rqyi", None),
    "unit": ("9dej-ik5u", None),
} | { # Using date_extract_y for floating timestamp
    f"annual_waste_summary_{y}": ("79s2-9ack", f"date_extract_y(report_year) = {y}")
    # pull 10 years of data based on current calendar year
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Extract and load selected TCEQ Socrata datasets into PostgreSQL."
    )
    parser.add_argument(
        "--dataset", "-d",
        choices=list(DATASETS.keys()),
        help="The specific dataset key to run. If omitted, all datasets will be loaded."
    )
    ...
    if args.dataset:
        ...
        print(f"🎯 Target Execution: Single dataset specified -> '{args.dataset}'")
    else:
        selected_datasets = DATASETS
        print("🌍 Target Execution: Loading all registered datasets")

    for name, config in selected_datasets.items():
